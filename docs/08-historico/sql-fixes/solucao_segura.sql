-- SOLUÇÃO SEGURA PARA O PROBLEMA DO TRIGGER DESABILITADO
-- Versão que evita conflitos de locks e não usa ALTER TABLE em triggers

-- 1. CRIAR FUNÇÃO PRINCIPAL COM TRATAMENTO DE ERROS ROBUSTO
...
-- 2. CRIAR TRIGGER PRINCIPAL (se não existir)
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        -- Criar o trigger
        EXECUTE 'CREATE TRIGGER trigger_calcular_progresso_termos
            AFTER INSERT OR UPDATE ON termos_ambientais
            FOR EACH ROW
            EXECUTE FUNCTION calcular_progresso_metas()';
        
        RAISE NOTICE '✅ Trigger trigger_calcular_progresso_termos criado com sucesso';
    ELSE
        RAISE NOTICE 'ℹ️ Trigger trigger_calcular_progresso_termos já existe';
    END IF;
END $$;
...
BEGIN
    ...
    RAISE NOTICE '🔍 STATUS DO TRIGGER:';
...
-- 4. RECALCULAR PROGRESSO EXISTENTE (versão segura)
DO $$
DECLARE
    termo_record RECORD;
    ...
    LOOP
        BEGIN
            ...
            IF processed_count % 10 = 0 THEN
                RAISE NOTICE '   Processados: %/%', processed_count, total_termos;
            END IF;
            
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '⚠️ Erro ao processar termo %: %', termo_record.id, SQLERRM;
        END;
    END LOOP;
    
    RAISE NOTICE '✅ Recalculo concluído! Processados: %', processed_count;
END $$;

-- 5. VERIFICAÇÃO FINAL DO PROGRESSO
SELECT 
