-- ===================================================================
-- RECRIAR TODOS OS TRIGGERS DE METAS - COMPLETO
-- ===================================================================

-- 1. REMOVER TRIGGERS EXISTENTES
DROP TRIGGER IF EXISTS trigger_calcular_progresso_termos_insert ON termos_ambientais;
DROP TRIGGER IF EXISTS trigger_calcular_progresso_lvs_insert ON lv_residuos;
...
BEGIN
    ...
    IF meta_info.id IS NOT NULL THEN
        ...
        INSERT INTO progresso_metas (
            ...
            meta_id_val, tma_id_val, periodo_val, ano_val, mes_val, semana_val, dia_val,
...
BEGIN
    ...
    periodo_val := 'mensal';
    ano_val := EXTRACT(YEAR FROM NEW.data_inspecao);
    mes_val := EXTRACT(MONTH FROM NEW.data_inspecao);
    semana_val := EXTRACT(WEEK FROM NEW.data_inspecao);
    dia_val := EXTRACT(DAY FROM NEW.data_inspecao);
    tma_id_val := NEW.inspetor_id;
    
    -- Buscar meta de LVs para o período
    SELECT m.id, m.meta_quantidade 
    INTO meta_info 
    FROM metas m
    ...
    IF meta_info.id IS NOT NULL THEN
        ...
        -- Calcular quantidade atual (soma de lv_residuos e lvs)
        SELECT COALESCE(
            (SELECT COUNT(*) FROM lv_residuos 
             WHERE inspetor_id = tma_id_val
               AND EXTRACT(YEAR FROM data_inspecao) = ano_val
               AND EXTRACT(MONTH FROM data_inspecao) = mes_val), 0
        ) + COALESCE(
...
BEGIN
    ...
    -- Determinar valores baseados na rotina
    periodo_val := 'mensal';
    ...
    IF meta_info.id IS NOT NULL THEN
        ...
        WHERE responsavel_id = tma_id_val
...
CREATE TRIGGER trigger_calcular_progresso_lvs_insert_residuos
AFTER INSERT ON lv_residuos
FOR EACH ROW
EXECUTE FUNCTION calcular_progresso_lvs();

-- Trigger para lvs
CREATE TRIGGER trigger_calcular_progresso_lvs_insert_lvs
AFTER INSERT ON lvs
FOR EACH ROW
EXECUTE FUNCTION calcular_progresso_lvs();
...
WHERE trigger_schema = 'public'
  AND event_object_table IN ('termos_ambientais', 'lv_residuos', 'lvs', 'atividades_rotina')
